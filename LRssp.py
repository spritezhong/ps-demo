...
from SSPwork import SSPWorkerClass
class LRsspmodel():
	def __init__(self,epochs,id,ip,port,role,client_id,servernum):
		self.worker = SSPWorkerClass(id,ip,port,role, client_id, servernum) #id, '127.0.0.1', 8001, 'worker', 0, 1,
	...
	def train(self):
		...
		for i in range(self.epochs):
			self.worker.wait(self.worker.pullwithclock(self.keylist, self.vallist,i))
			# print('continue')
